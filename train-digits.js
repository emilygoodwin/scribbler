#!/usr/bin/env node
// One-time offline training script for digit recognition.
// Run: node train-digits.js
// Outputs: model-digits/model.json + model-digits/*.bin
//
...
const EMNIST_IMAGES = path.join(__dirname, 'data', 'emnist-digits-train-images-idx3-ubyte');
const EMNIST_LABELS = path.join(__dirname, 'data', 'emnist-digits-train-labels-idx1-ubyte');
const OUT_DIR       = path.join(__dirname, 'model-digits');
...
function buildModel() {
  ...
  m.add(tf.layers.dense({ units: 10, activation: 'softmax' }));  // 10 digit classes (0–9)
  m.compile({ optimizer: 'adam', loss: 'sparseCategoricalCrossentropy', metrics: ['acc'] });
  return m;
}

async function main() {
  console.log('Loading EMNIST digits data…');
  ...
  // EMNIST digits labels are already 0-based (0–9), no offset needed
  const ys = tf.tensor1d(Array.from(lbls.data.slice(0, n)), 'float32');
  ...
  console.log('Push model-digits/ alongside index.html and the spy game will activate.');
}
